...
<el-divider v-if="toolBtn" /> -->
    <el-row :gutter="10" class="mb8 row" v-if="backBtn === true || addBtn == true || toolBtn === true">
      <el-col :span="1.5" v-if="backBtn === true">
        <el-button plain icon="arrowLeftBold" size="small" @click="handleCloseTable">返回</el-button>
      </el-col>
      <el-col :span="1.5" v-if="addBtn == true">
        <el-button size="small" type="primary" plain icon="Plus" @click="handleAddFile"
          v-hasPermi="['recmd:fileList:add']">添加
        </el-button>
      </el-col>
      <el-col :span="1.5" v-if="addBtn == true">
        <el-button size="small" type="danger" plain icon="delete" :disabled="single" @click="handleRemoveFileJson"
          v-hasPermi="['recmd:fileList:remove']">移除
        </el-button>
      </el-col>
      <right-toolbar @queryTable="refreshFileList()" :columns="columns" v-if="toolBtn"></right-toolbar>
    </el-row>
    <el-table v-loading="loading" :data="fileList" class="file-table"
      :default-sort="{ prop: 'xssx', order: 'ascending' }" @selection-change="handleSelectionChange" border>
      <el-table-column v-if="addBtn == true" type="selection" :width="computeWidth(45)" align="center" />
      <el-table-column align="center" :width="computeWidth(60)">
        <template #default="scope">
          <span class="reviewMode content-style">{{ scope.$index + 1 }}</span>
        </template>
      </el-table-column>
      <el-table-column label="ID" align="center" prop="id" :width="computeWidth(80)" v-if="columns[0].visible">
      ...
      <el-table-column label="年度" prop="nd" :show-overflow-tooltip="true" :width="computeWidth(80)" v-if="columns[2].visible && showNdCol" sortable>
        <template #default="scope"><span class="reviewMode content-style">{{ scope.row.nd }}</span></template>
      </el-table-column>
      <el-table-column label="类型" align="center" prop="ext" :width="computeWidth(120)" v-if="columns[3].visible">
      ...
      <el-table-column label="是否必须" align="center" :width="computeWidth(80)" prop="required"
        v-if="columns[4].visible && !viewType">
        <template #default="scope">
          <dict-tag :options="sys_yes_no" :value="scope.row.required === true ? 'Y' : 'N'" />
        </template>
      </el-table-column>
      <el-table-column label="大小限制" align="center" :width="computeWidth(80)" prop="maxSize"
        v-if="columns[5].visible && !viewType">
        <template #default="scope">
          <span class="reviewMode content-style">{{ scope.row.maxSize }}M</span>
        </template>
      </el-table-column>
      <el-table-column label="顺序" align="center" :width="computeWidth(80)" prop="xssx" v-if="columns[6].visible" sortable>
        <template #default="scope">
          <span class="reviewMode content-style">{{ scope.row.xssx }}</span>
        </template>
      </el-table-column>
      <el-table-column label="预览" prop="path" v-if="columns[7].visible" :show-overflow-tooltip="true"
        :width="computeWidth(200)">
        <template #default="scope">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" :width="computeWidth(220)"
        fixed="right" v-if="!viewType">
        <template #default="scope">
          <el-upload :ref="'uploadFile' + scope.row.id" action="scope.row.id" :limit="1" :data=scope.row
            ...
            <el-button size="small" class="w50" type="danger" :loading="upload.isUploading"
              v-if="scope.row.del_url && scope.row.path" v-hasPermi="['recmd:fileList:delFile']"
              @click="removeFiles(scope.row)">删除
            </el-button>
          </el-upload>
        </template>
      </el-table-column>
...
const checkBtnSelectedRow = ref([]);
const single = ref(true);
const multiple = ref(true);

function handleSelectionChange(selection) {
  checkBtnSelectedRow.value = selection;
...
function handleRemoveFileJson() {
  let row = checkBtnSelectedRow.value;

  if (row !== undefined) {
    if (row.length > 1) {
      proxy.$modal.msgSuccess("一次只能删除一项")
      return;
    }
    proxy.$modal
      .confirm("删除操作将删除文件及文件信息，且不可恢复。确实需要删除么？")
      .then(function () {
        let param = row[0];
        param.removeJson = true;
        removeFiles(param);
      })
  }
}
