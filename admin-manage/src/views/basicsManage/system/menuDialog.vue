    ...
    >
      <div class="full-block scroll-y">
        <el-form ref="form" label-width="120px" :rules="rules" :model="model">
          ...
          <el-form-item
            v-show="model.menuType !== $cv.MENU_TYPE.BUTTON"
            prop="component"
            label="前端组件："
          >
            <el-input
          ...
          <el-form-item
            v-show="model.menuType === $cv.MENU_TYPE.TOP_MENU"
            label="默认跳转地址："
          >
            <el-input
              class="w230"
          ...
          <el-form-item v-show="!show" label="授权策略：">
            <el-radio-group v-model="model.permsType" :disabled="disableSubmit">
              <el-radio :label="$cv.PERMS_TYPE.SHOW"
                >可见/可访问(授权后可见/可访问)</el-radio
              >
              <el-radio :label="$cv.PERMS_TYPE.DISABLE"
                >可编辑(未授权时禁用)</el-radio
              >
            </el-radio-group>
          </el-form-item>
          <el-form-item v-show="!show" label="状态：">
            <el-radio-group v-model="model.status" :disabled="disableSubmit">
              ...
              <el-radio :label="$cv.BUTTONS_PERMS_STATUS.DISABLE"
                >无效</el-radio
              >
            </el-radio-group>
          </el-form-item>
          ...
          <el-form-item v-show="show" label="打开方式：">
            <el-switch
              active-text="外部"
              inactive-text="内部"
              v-model="model.internalOrExternal"
              :disabled="disableSubmit"
            ></el-switch>
          </el-form-item>
        </el-form>
      ...
      <span slot="footer">
        <el-button type="primary" :disabled="disableSubmit" @click="handleSave"
          >保存</el-button
        >
  ...
  methods: {
    ...
    add() {
      ...
      this.edit({
        ...
        sortNo: 1.0,
        route: true,
        menuType: this.$cv.MENU_TYPE.TOP_MENU
      });
    },
    /**
     * 编辑
    ...
    edit(record) {
      ...
      this.menuLabel =
        record.menuType === this.$cv.MENU_TYPE.BUTTON ?
          '按钮/权限' :
          '菜单名称';
      this.visibleDialog = true;
      const list = Common.CloneDeep(this.$parent.dataList);
      this.treeData = Common.depthFirstSearchMap(
        list,
        (item) => {
          if (item['menuType'] === this.$cv.MENU_TYPE.BUTTON) {
            return false;
          } else {
    ...
    handleSave() {
      this.$refs.form.validate((val) => {
        if (val) {
          ...
          if (
            (this.model.menuType === this.$cv.MENU_TYPE.SEC_MENU ||
              this.model.menuType === this.$cv.MENU_TYPE.BUTTON) &&
            !this.model.parentId
          ) {
